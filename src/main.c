...
int add_iptables_rules()
{
    ...
    sprintf(cmd, "iptables -t raw -I PREROUTING -p icmp -j DROP");
    system(cmd);
    ...
    if (opt_protect_http_protocol) {
        ...
        sprintf(cmd, "iptables -t raw -I PREROUTING -p tcp --sport 80 --tcp-flags RST RST -j NFQUEUE --queue-num %d", NF_QUEUE_NUM);
        system(cmd);
        ...
        sprintf(cmd, "iptables -t mangle -I POSTROUTING -p tcp --dport 80 --tcp-flags SYN,ACK,RST ACK -m mark ! --mark %d -m u32 --u32 '0>>22&0x3C@ 12>>26&0x3C@ 0=0x47455420' -j NFQUEUE --queue-num %d", MARK, NF_QUEUE_NUM);
        system(cmd);
        // intercept outgoing HTTP 'POST' request
        sprintf(cmd, "iptables -t mangle -I POSTROUTING -p tcp --dport 80 --tcp-flags SYN,ACK,RST ACK -m mark ! --mark %d -m u32 --u32 '0>>22&0x3C@ 12>>26&0x3C@ 0=0x504F5354' -j NFQUEUE --queue-num %d", MARK, NF_QUEUE_NUM);
        system(cmd);
        // intercept incoming HTTP 'HTTP' response
        sprintf(cmd, "iptables -t mangle -I PREROUTING -p tcp --sport 80 --tcp-flags SYN,ACK,RST ACK -m u32 --u32 '0>>22&0x3C@ 12>>26&0x3C@ 0=0x48545450' -j NFQUEUE --queue-num %d", NF_QUEUE_NUM);
        system(cmd);
    }
    if (opt_protect_https_protocol) {
        // monitor incoming RST
        sprintf(cmd, "iptables -t raw -I PREROUTING -p tcp --sport 443 --tcp-flags RST RST -j NFQUEUE --queue-num %d", NF_QUEUE_NUM);
        system(cmd);
        // monitor outgoing SYN
        sprintf(cmd, "iptables -t mangle -I POSTROUTING -p tcp --dport 443 --tcp-flags SYN,ACK SYN -m mark ! --mark %d -j NFQUEUE --queue-num %d", MARK, NF_QUEUE_NUM);
        system(cmd);
        // monitor incoming SYN-ACK
        sprintf(cmd, "iptables -t raw -I PREROUTING -p tcp --sport 443 --tcp-flags SYN,ACK SYN,ACK -j NFQUEUE --queue-num %d", NF_QUEUE_NUM);
        system(cmd);
        // intercept outgoing ACK
        sprintf(cmd, "iptables -t mangle -I POSTROUTING -p tcp --dport 443 --tcp-flags SYN,ACK,RST ACK -m mark ! --mark %d -m length --length 0:80 -j NFQUEUE --queue-num %d", MARK, NF_QUEUE_NUM);
        system(cmd);
        // intercept outgoing HTTPS ClientHello message
        sprintf(cmd, "iptables -t mangle -I POSTROUTING -p tcp --dport 443 --tcp-flags SYN,ACK,RST ACK -m mark ! --mark %d -m u32 --u32 '0>>22&0x3C@ 12>>26&0x3C@ 0&0xFFFF0000=0x16030000' -j NFQUEUE --queue-num %d", MARK, NF_QUEUE_NUM);
        system(cmd);
        // intercept incoming HTTPS ServerHello message
        sprintf(cmd, "iptables -t mangle -I PREROUTING -p tcp --sport 443 --tcp-flags SYN,ACK,RST ACK -m u32 --u32 '0>>22&0x3C@ 12>>26&0x3C@ 0&0xFFFF0000=0x16030000' -j NFQUEUE --queue-num %d", NF_QUEUE_NUM);
        system(cmd);
    }
    if (opt_protect_dns_protocol) {
        // monitor incoming RST
        sprintf(cmd, "iptables -t raw -I PREROUTING -p tcp --sport 53 --tcp-flags RST RST -j NFQUEUE --queue-num %d", NF_QUEUE_NUM);
        system(cmd);
        // monitor outgoing SYN
        sprintf(cmd, "iptables -t mangle -I POSTROUTING -p tcp --dport 53 --tcp-flags SYN,ACK SYN -m mark ! --mark %d -j NFQUEUE --queue-num %d", MARK, NF_QUEUE_NUM);
        system(cmd);
        // monitor incoming SYN-ACK
        sprintf(cmd, "iptables -t raw -I PREROUTING -p tcp --sport 53 --tcp-flags SYN,ACK SYN,ACK -j NFQUEUE --queue-num %d", NF_QUEUE_NUM);
        system(cmd);
        // intercept outgoing ACK
        sprintf(cmd, "iptables -t mangle -I POSTROUTING -p tcp --dport 53 --tcp-flags SYN,ACK,RST ACK -m mark ! --mark %d -m length --length 0:80 -j NFQUEUE --queue-num %d", MARK, NF_QUEUE_NUM);
        system(cmd);
        // monitor outgoing DNS UDP request
        sprintf(cmd, "iptables -t raw -I POSTROUTING -o lo -p udp --dport 53 -j NFQUEUE --queue-num %d", NF_QUEUE_NUM);
        system(cmd);
        // monitor incoming DNS UDP response
        sprintf(cmd, "iptables -t raw -I PREROUTING ! -i lo -p udp --sport 53 -j NFQUEUE --queue-num %d", NF_QUEUE_NUM);
        system(cmd);
        // monitor outgoing DNS TCP request
        sprintf(cmd, "iptables -t mangle -I POSTROUTING -p tcp --dport 53 --tcp-flags SYN,ACK,RST ACK -m mark ! --mark %d -j NFQUEUE --queue-num %d", MARK, NF_QUEUE_NUM);
        system(cmd);
        // monitor incoming DNS TCP response
        sprintf(cmd, "iptables -t mangle -I PREROUTING -p tcp --sport 53 --tcp-flags SYN,ACK,RST ACK -m length --length 80:0xffff -j NFQUEUE --queue-num %d", NF_QUEUE_NUM);
        system(cmd);
    }
    if (opt_protect_vpn_protocol) {
        // monitor incoming RST
        sprintf(cmd, "iptables -t raw -I PREROUTING -p tcp --sport %d --tcp-flags RST RST -j NFQUEUE --queue-num %d", opt_vpn_port, NF_QUEUE_NUM);
        system(cmd);
        // monitor outgoing SYN
        sprintf(cmd, "iptables -t mangle -I POSTROUTING -p tcp --dport %d --tcp-flags SYN,ACK SYN -m mark ! --mark %d -j NFQUEUE --queue-num %d", opt_vpn_port, MARK, NF_QUEUE_NUM);
        system(cmd);
        // monitor incoming SYN-ACK
        sprintf(cmd, "iptables -t raw -I PREROUTING -p tcp --sport %d --tcp-flags SYN,ACK SYN,ACK -j NFQUEUE --queue-num %d", opt_vpn_port, NF_QUEUE_NUM);
        system(cmd);
        // intercept outgoing ACK
        //sprintf(cmd, "iptables -t mangle -A POSTROUTING -p tcp --dport %d --tcp-flags SYN,ACK,RST ACK -m mark ! --mark %d -m length --length 0:80 -j NFQUEUE --queue-num %d", opt_vpn_port, MARK, NF_QUEUE_NUM);
        //system(cmd);
        // monitor outgoing VPN packets (TODO: intercept less packet)
        sprintf(cmd, "iptables -t mangle -I POSTROUTING -p tcp --dport %d --tcp-flags SYN,ACK,RST ACK -m mark ! --mark %d -j NFQUEUE --queue-num %d", opt_vpn_port, MARK, NF_QUEUE_NUM);
        system(cmd);
    }
    if (opt_protect_tor_protocol) {
        // monitor incoming RST
        sprintf(cmd, "iptables -t raw -I PREROUTING -p tcp --sport %d --tcp-flags RST RST -j NFQUEUE --queue-num %d", opt_tor_port, NF_QUEUE_NUM);
        system(cmd);
        // monitor outgoing SYN
        ...
        //system(cmd);
        // monitor outgoing VPN packets (TODO: intercept less packet)
        sprintf(cmd, "iptables -t mangle -I POSTROUTING -p tcp --dport %d --tcp-flags SYN,ACK,RST ACK -m mark ! --mark %d -j NFQUEUE --queue-num %d", opt_tor_port, MARK, NF_QUEUE_NUM);
        system(cmd);
    }
    return 0;
}
...
int remove_iptables_rules()
{
    char cmd[1024];
    // discard incoming ICMP packet, otherwise there will be socker error "No route to host"
    sprintf(cmd, "iptables -t raw -D PREROUTING -p icmp -j DROP");
    system(cmd);
    if (opt_protect_http_protocol) {
        // monitor incoming RST
        sprintf(cmd, "iptables -t raw -D PREROUTING -p tcp --sport 80 --tcp-flags RST RST -j NFQUEUE --queue-num %d", NF_QUEUE_NUM);
        system(cmd);
        // monitor outgoing SYN
        sprintf(cmd, "iptables -t mangle -D POSTROUTING -p tcp --dport 80 --tcp-flags SYN,ACK SYN -m mark ! --mark %d -j NFQUEUE --queue-num %d", MARK, NF_QUEUE_NUM);
        system(cmd);
        // monitor incoming SYN-ACK
        sprintf(cmd, "iptables -t raw -D PREROUTING -p tcp --sport 80 --tcp-flags SYN,ACK SYN,ACK -j NFQUEUE --queue-num %d", NF_QUEUE_NUM);
        system(cmd);
        // intercept outgoing ACK
        sprintf(cmd, "iptables -t mangle -D POSTROUTING -p tcp --dport 80 --tcp-flags SYN,ACK,RST ACK -m mark ! --mark %d -m length --length 0:80 -j NFQUEUE --queue-num %d", MARK, NF_QUEUE_NUM);
        system(cmd);
        // intercept outgoing HTTP 'GET ' request
        sprintf(cmd, "iptables -t mangle -D POSTROUTING -p tcp --dport 80 --tcp-flags SYN,ACK,RST ACK -m mark ! --mark %d -m u32 --u32 '0>>22&0x3C@ 12>>26&0x3C@ 0=0x47455420' -j NFQUEUE --queue-num %d", MARK, NF_QUEUE_NUM);
        system(cmd);
        // intercept outgoing HTTP 'POST' request
        sprintf(cmd, "iptables -t mangle -D POSTROUTING -p tcp --dport 80 --tcp-flags SYN,ACK,RST ACK -m mark ! --mark %d -m u32 --u32 '0>>22&0x3C@ 12>>26&0x3C@ 0=0x504F5354' -j NFQUEUE --queue-num %d", MARK, NF_QUEUE_NUM);
        system(cmd);
        // intercept incoming HTTP 'HTTP' response
        sprintf(cmd, "iptables -t mangle -D PREROUTING -p tcp --sport 80 --tcp-flags SYN,ACK,RST ACK -m u32 --u32 '0>>22&0x3C@ 12>>26&0x3C@ 0=0x48545450' -j NFQUEUE --queue-num %d", NF_QUEUE_NUM);
        system(cmd);
    }
    if (opt_protect_https_protocol) {
        // monitor incoming RST
        sprintf(cmd, "iptables -t raw -D PREROUTING -p tcp --sport 443 --tcp-flags RST RST -j NFQUEUE --queue-num %d", NF_QUEUE_NUM);
        system(cmd);
        // monitor outgoing SYN
        sprintf(cmd, "iptables -t mangle -D POSTROUTING -p tcp --dport 443 --tcp-flags SYN,ACK SYN -m mark ! --mark %d -j NFQUEUE --queue-num %d", MARK, NF_QUEUE_NUM);
        system(cmd);
        // monitor incoming SYN-ACK
        sprintf(cmd, "iptables -t raw -D PREROUTING -p tcp --sport 443 --tcp-flags SYN,ACK SYN,ACK -j NFQUEUE --queue-num %d", NF_QUEUE_NUM);
        system(cmd);
        // intercept outgoing ACK
        sprintf(cmd, "iptables -t mangle -D POSTROUTING -p tcp --dport 443 --tcp-flags SYN,ACK,RST ACK -m mark ! --mark %d -m length --length 0:80 -j NFQUEUE --queue-num %d", MARK, NF_QUEUE_NUM);
        system(cmd);
        // intercept outgoing HTTPS ClientHello message
        sprintf(cmd, "iptables -t mangle -D POSTROUTING -p tcp --dport 443 --tcp-flags SYN,ACK,RST ACK -m mark ! --mark %d -m u32 --u32 '0>>22&0x3C@ 12>>26&0x3C@ 0&0xFFFF0000=0x16030000' -j NFQUEUE --queue-num %d", MARK, NF_QUEUE_NUM);
        system(cmd);
        // intercept incoming HTTPS ServerHello message
        sprintf(cmd, "iptables -t mangle -D PREROUTING -p tcp --sport 443 --tcp-flags SYN,ACK,RST ACK -m u32 --u32 '0>>22&0x3C@ 12>>26&0x3C@ 0&0xFFFF0000=0x16030000' -j NFQUEUE --queue-num %d", NF_QUEUE_NUM);
        system(cmd);
    }
    if (opt_protect_dns_protocol) {
        // monitor incoming RST
        sprintf(cmd, "iptables -t raw -D PREROUTING -p tcp --sport 53 --tcp-flags RST RST -j NFQUEUE --queue-num %d", NF_QUEUE_NUM);
        system(cmd);
        // monitor outgoing SYN
        sprintf(cmd, "iptables -t mangle -D POSTROUTING -p tcp --dport 53 --tcp-flags SYN,ACK SYN -m mark ! --mark %d -j NFQUEUE --queue-num %d", MARK, NF_QUEUE_NUM);
        system(cmd);
        // monitor incoming SYN-ACK
        sprintf(cmd, "iptables -t raw -D PREROUTING -p tcp --sport 53 --tcp-flags SYN,ACK SYN,ACK -j NFQUEUE --queue-num %d", NF_QUEUE_NUM);
        system(cmd);
        // intercept outgoing ACK
        sprintf(cmd, "iptables -t mangle -D POSTROUTING -p tcp --dport 53 --tcp-flags SYN,ACK,RST ACK -m mark ! --mark %d -m length --length 0:80 -j NFQUEUE --queue-num %d", MARK, NF_QUEUE_NUM);
        system(cmd);
        // monitor outgoing DNS UDP request
        sprintf(cmd, "iptables -t raw -D POSTROUTING -o lo -p udp --dport 53 -j NFQUEUE --queue-num %d", NF_QUEUE_NUM);
        system(cmd);
        // monitor incoming DNS UDP response
        sprintf(cmd, "iptables -t raw -D PREROUTING ! -i lo -p udp --sport 53 -j NFQUEUE --queue-num %d", NF_QUEUE_NUM);
        system(cmd);
        // monitor outgoing DNS TCP request
        sprintf(cmd, "iptables -t mangle -D POSTROUTING -p tcp --dport 53 --tcp-flags SYN,ACK,RST ACK -m mark ! --mark %d -j NFQUEUE --queue-num %d", MARK, NF_QUEUE_NUM);
        system(cmd);
        // monitor incoming DNS TCP response
        sprintf(cmd, "iptables -t mangle -D PREROUTING -p tcp --sport 53 --tcp-flags SYN,ACK,RST ACK -m length --length 80:0xffff -j NFQUEUE --queue-num %d", NF_QUEUE_NUM);
        system(cmd);
    }
    if (opt_protect_vpn_protocol) {
        // monitor incoming RST
        sprintf(cmd, "iptables -t raw -D PREROUTING -p tcp --sport %d --tcp-flags RST RST -j NFQUEUE --queue-num %d", opt_vpn_port, NF_QUEUE_NUM);
        system(cmd);
        // monitor outgoing SYN
        sprintf(cmd, "iptables -t mangle -D POSTROUTING -p tcp --dport %d --tcp-flags SYN,ACK SYN -m mark ! --mark %d -j NFQUEUE --queue-num %d", opt_vpn_port, MARK, NF_QUEUE_NUM);
        system(cmd);
        // monitor incoming SYN-ACK
        sprintf(cmd, "iptables -t raw -D PREROUTING -p tcp --sport %d --tcp-flags SYN,ACK SYN,ACK -j NFQUEUE --queue-num %d", opt_vpn_port, NF_QUEUE_NUM);
        system(cmd);
        // intercept outgoing ACK
        //sprintf(cmd, "iptables -t mangle -D POSTROUTING -p tcp --dport %d --tcp-flags SYN,ACK,RST ACK -m mark ! --mark %d -m length --length 0:80 -j NFQUEUE --queue-num %d", opt_vpn_port, MARK, NF_QUEUE_NUM);
        //system(cmd);
        // monitor outgoing VPN packets (TODO: intercept less packet)
        sprintf(cmd, "iptables -t mangle -D POSTROUTING -p tcp --dport %d --tcp-flags SYN,ACK,RST ACK -m mark ! --mark %d -j NFQUEUE --queue-num %d", opt_vpn_port, MARK, NF_QUEUE_NUM);
        system(cmd);
    }
    if (opt_protect_tor_protocol) {
        // monitor incoming RST
        sprintf(cmd, "iptables -t raw -D PREROUTING -p tcp --sport %d --tcp-flags RST RST -j NFQUEUE --queue-num %d", opt_tor_port, NF_QUEUE_NUM);
        system(cmd);
        // monitor outgoing SYN
        sprintf(cmd, "iptables -t mangle -D POSTROUTING -p tcp --dport %d --tcp-flags SYN,ACK SYN -m mark ! --mark %d -j NFQUEUE --queue-num %d", opt_tor_port, MARK, NF_QUEUE_NUM);
        system(cmd);
        // monitor incoming SYN-ACK
        sprintf(cmd, "iptables -t raw -D PREROUTING -p tcp --sport %d --tcp-flags SYN,ACK SYN,ACK -j NFQUEUE --queue-num %d", opt_tor_port, NF_QUEUE_NUM);
        system(cmd);
        // intercept outgoing ACK
        //sprintf(cmd, "iptables -t mangle -D POSTROUTING -p tcp --dport %d --tcp-flags SYN,ACK,RST ACK -m mark ! --mark %d -m length --length 0:80 -j NFQUEUE --queue-num %d", opt_tor_port, MARK, NF_QUEUE_NUM);
        //system(cmd);
        // monitor outgoing VPN packets (TODO: intercept less packet)
        sprintf(cmd, "iptables -t mangle -D POSTROUTING -p tcp --dport %d --tcp-flags SYN,ACK,RST ACK -m mark ! --mark %d -j NFQUEUE --queue-num %d", opt_tor_port, MARK, NF_QUEUE_NUM);
        system(cmd);
    }
    return 0;
}
